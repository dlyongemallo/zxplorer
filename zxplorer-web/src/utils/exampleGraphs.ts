...
/**
 * Create the default example graph from ZXLive (4-qubit quantum circuit)
 *
 * This function was previously in quizx-wasm bindings but has been moved
 * to the application layer as it's application-specific, not a core QuiZX binding.
 */
export function createExampleGraph(): ZXGraph {
  ...
  // Vertex list: (id, qubit, type)
  // VertexType: 0=Boundary, 1=Z, 2=X, 3=H
  const vlist = [
  ...
  // Edge list: (id1, id2, edge_type)
  // EdgeType: 0=Normal, 1=Hadamard
  const elist = [
  ...
  // Add edges from inputs to first internal nodes
  for (let i = 0; i < qubits; i++) {
    g.add_edge_with_type(vertexIds[i], vertexIds[i + qubits], 0)
  }

  // Add edges from last internal nodes to outputs
  for (let i = 0; i < qubits; i++) {
