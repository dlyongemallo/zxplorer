...
impl ZXGraph {
    ...
    /// Add an edge between two vertices (edge_type: 0=Simple, 1=Hadamard)
    ...
    pub fn add_edge_with_type(&mut self, source: usize, target: usize, edge_type: u8) {
        let et = match edge_type {
            1 => EType::H,  // Hadamard edge
            _ => EType::N,  // Simple edge (default)
        };
    ...
    pub fn set_edge_type(&mut self, source: usize, target: usize, edge_type: u8) -> Result<(), String> {
        ...
        let et = match edge_type {
            0 => EType::N,  // Simple edge
            1 => EType::H,  // Hadamard edge
            _ => return Err("Invalid edge type. Must be 0 or 1".to_string()),
        };

        // Remove and re-add edge with new type
        self.inner.remove_edge(source, target);
        self.inner.add_edge_with_type(source, target, et);
        Ok(())
    }
    ...
    pub fn set_vertex_phase(&mut self, vertex: usize, phase_str: &str) -> Result<(), String> {
        ...
        // Parse phase string as "numerator/denominator" or just "numerator"
    ...
    /// Get all vertices as a JSON string
    pub fn get_vertices_json(&self) -> String {
        let vertices: Vec<VertexInfo> = self.inner.vertices()
            .map(|v| {
                ...
                VertexInfo {
                    id: v,
                    vertex_type: vt as u8,  // Convert enum to u8
    ...
    /// Get all edges as a JSON string
    pub fn get_edges_json(&self) -> String {
        let edges: Vec<EdgeInfo> = self.inner.edges()
            .map(|(s, t, et)| EdgeInfo {
                ...
                target: t,
                edge_type: match et {
                    EType::N => 0,    // Simple edge
                    EType::H => 1,    // Hadamard edge
                    EType::Wio => 0,  // Treat W I/O same as simple for now
                }
            })
            .collect();
    ...
    /// Apply spider fusion rule (simplification)
    ...
    /// Apply all spider fusions until no more can be applied
    ...
    /// Apply local complementation simplification
    pub fn simplify_local_comp(&mut self) -> bool {
        local_comp_simp(&mut self.inner)
    }

    /// Apply spider fusion simplification
    ...
    pub fn to_json(&self) -> String {
        // Serialize vertices and edges together
    ...
    /// Get a simple string representation of the graph
...
// NOTE: create_example_graph() was removed from bindings as it's application-specific.
// It should be implemented in the React app (ZXDiagram.tsx or a separate utilities file)
// when the web app is ported in future commits.
